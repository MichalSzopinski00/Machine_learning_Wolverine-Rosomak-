...
def main():
    ...
    features_train, features_test, target_train, target_test = train_test_split(
        features_df, labels_df, random_state=0, stratify=labels_df
    )

    pipe = Pipeline([("classifier", RandomForestClassifier())])
    print("initializing grid search")
    multi_grid_search_gs = GridSearchCV(
        pipe, hyper_parameters.search_space, verbose=1, n_jobs=-1, scoring="f1_micro"
    )
    best_models_gs = multi_grid_search_gs.fit(
        features_train, target_train.values.flatten()
    )
